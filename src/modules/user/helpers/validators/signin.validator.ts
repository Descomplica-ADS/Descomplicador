...
export async function signinValidator(
  ...
  try {
    const bodyValidate = joi.object({
      ...
      email: joi.string().email().min(12).max(60).messages({
      ...
      refresh: joi.bool().required().messages({
        'bool.base': 'O campo mantenha-me conectado precisa ser um booleano',
        'bool.required': 'O campo mantenha-me conectado é necessário',
      }),
    });
    ...
    if (!req.body.username && !req.body.email)
      throw new HTTPError(
        'É necessário informar um e-mail ou nome de usuário',
        400,
      );
